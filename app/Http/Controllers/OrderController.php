...
class OrderController extends MainController {
	...
	{
		if(str_contains($_SERVER['REQUEST_URI'], 'edit/SO')) $jr='SO';
		if(str_contains($_SERVER['REQUEST_URI'], 'edit/PO')) $jr='PO';
		$data = [
			'jr' 		=> $jr,
      	    'id' 		=> $id,
		...
		if ($jr == 'SO') {
			$data = array_merge($data, [
				...
				// 'mSalesman'	=> json_encode($this->modalData(['modSalesman'])),
				// 'mCustomer'	=> json_encode($this->modalData(['modAccount-AR'])),
		...
		*/
		...
		//get data
		$res = $this->api('GET', "api/$jr/$id" );
		//dd($res);
		if(!empty($res)) {
			$data['data']		= $res;
			$data['griddata']	= $this->api('GET', "api/orderdetail/" );
		} else {
			$data['data']		= [];
			$data['griddata']	= [];
		}
